            ...
            it("should get the correct repo dictionaries and create repository objects from them") {
                ...
                waitUntil(action: { (done) in
                    store.getRepositoriesWithCompletion {
                        expect(store.githubReposArray.count).to(equal(2))
                        
                        let repo1 = store.githubReposArray[0]
                        expect(repo1.fullName).to(equal("mojombo/grit"))
                        expect(repo1.repositoryID).to(equal("1"))
                        expect(repo1.html).to(equal(NSURL(string: "https://github.com/mojombo/grit")))
                        
                        let repo2 = store.githubReposArray[1]
                        expect(repo2.fullName).to(equal("wycats/merb-core"))
                        expect(repo2.repositoryID).to(equal("26"))
                        expect(repo2.html).to(equal(NSURL(string: "https://github.com/wycats/merb-core")))
                        done()
                    }
                })
            }
